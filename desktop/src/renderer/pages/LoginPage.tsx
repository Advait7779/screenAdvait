  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.savedLicenseUsed) {
          setEnterDifferentLicense(true);
        }
        const errorMsg = res.savedLicenseUsed
          ? 'Login failed. Check password or verify your license key below.'
              ...
              <div className="rounded-md border border-green-200 bg-green-50 px-3 py-2.5 flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setEnterDifferentLicense(true)}
                      className="text-[11px] font-semibold text-green-800 hover:text-green-950 underline cursor-pointer"
                    >
                      Change Key
                    </button>
                  </div>
