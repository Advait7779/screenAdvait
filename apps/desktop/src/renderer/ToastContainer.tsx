  ...
  return (
    <div
      className={`pointer-events-auto flex items-center gap-2.5 px-4 py-2 rounded-full shadow-lg border backdrop-blur-md transition-all duration-300 ease-out transform ${
        ...
        isSuccess
          ? 'bg-emerald-50/95 border-emerald-200 text-emerald-900 font-medium text-xs shadow-emerald-500/10'
          : isError
          ? 'bg-red-50/95 border-red-200 text-red-900 font-medium text-xs shadow-red-500/10'
          : 'bg-sky-50/95 border-sky-200 text-sky-900 font-medium text-xs shadow-sky-500/10'
      }`}
    >
      ...
      {!isSuccess && !isError && <Info className="w-4 h-4 text-sky-600 shrink-0" />}
